const AudioUtils = (function(browser, sounds) {
  'use strict';

  const throwIfMissing = ()=> { throw new Error('Missing parameter'); };

  return {
    create: function(file = throwIfMissing, commentator = UserPrefs.defaults.commentator, volume = UserPrefs.defaults.volume) {
    ...
    getRandom: function(key = throwIfMissing, commentator = UserPrefs.defaults.commentator) {
      if (!sounds) { return; }
    ...
    getGeneric: function(key = throwIfMissing, commentator = UserPrefs.defaults.commentator) {
